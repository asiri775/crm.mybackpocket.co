...
class Dashboard
{
    ...
    public function getTotalLeadsStats(): array
    {
        ...
        $leadPipelineStages = DB::table('lead_pipeline_stages')->get();
        foreach ($leadPipelineStages as $leadPipelineStage) {
            $leadStateData[$leadPipelineStage->code] = [
                'over_time' => $this->leadReporting->getTotalLeadsOverTime('auto', [$leadPipelineStage->id]),
                'name' => $leadPipelineStage->name
            ];
        }

        return $leadStateData;
