...
class EmailController extends Controller
{
    ...
    public function store()
    {
        ...
        if (array_key_exists('reply', $emailData)) {

            $user = auth()->user();

            $userName = $user->first_name . " " . $user->last_name;

            $hostProfileUrl = env('MAIN_APP_URL') . '/profile/' . urlencode($user->user_name);
            $greeting = "Hello";
            $message = nl2br(string: htmlspecialchars($emailData['reply']));
            $replyButton = '<a href="' . env('APP_URL') . '" style="display: inline-block; padding: 10px 20px; background-color: #FEBF00; color: #000; text-decoration: none; border-radius: 5px;">Reply</a>';

            $emailData['reply'] = "" .
                $greeting . "<br><br>" .
                $message . "<br><br>" .
                "From: <a href='$hostProfileUrl'>" . htmlspecialchars($userName) . "</a><br><br>" .
                $replyButton;
        }
