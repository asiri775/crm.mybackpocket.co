            ...
            >
                ...
                >
                    <span class="cursor-pointer truncate rounded">
                        @{{ valueLabel ? valueLabel : inputValue.map(item => `${item.value}(${item.label})`).join(', ').length > 50 ? inputValue.map(item => `${item.value}(${item.label})`).join(', ').substring(0, 50) + '...' : inputValue.map(item => `${item.value}(${item.label})`).join(', ') }}
                    </span>
